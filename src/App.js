...
function App() {
  ...
  const [messages, setMeassges] = useState([]);
  const [username, setUsername] = useState('');
  useEffect(()=>{
    db.collection('messages').orderBy('timestamp','desc').onSnapshot(snapshot=>{
        setMeassges(snapshot.docs.map(doc=>({id:doc.id,message:doc.data()})))
    });
  },[]);
